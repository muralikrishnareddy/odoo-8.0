...
class ZbExcelExport(ExcelExport):
    ...
    def from_data(self, fields, rows):
        ...
        header_bold = xlwt.easyxf("font: bold on; pattern: pattern solid, fore_colour gray25;borders: top thin, bottom thin, left thin, right thin;")
        font = xlwt.Font()
        font.bold = True
        style.font = font
        ignore_index = []
        ...
        for i, fieldname in enumerate(fields):
            if fieldname.get('header_data_id', False):
                ...
                worksheet.write(0, i-count, field_name, header_bold)
                if i==0:
            ...
            self.from_data(data.get('headers', []), data.get('rows', [])),
                           headers=[
                                    ('Content-Disposition', 'attachment; filename="%s"'
                                        % (self.filename(filename) or self.filename(model))),#% data.get('model', 'Export.xls')),
